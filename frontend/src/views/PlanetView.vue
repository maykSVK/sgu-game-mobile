  ...
  <div>
    ...
    <div class="dash-page">
      ...
      <div class="dash-content">
        ...
        <div v-if="loading && !planetData" class="loading-box">
          Nacítám expedice...
        </div>

        <!-- GRID PRO INFOBOXY -->
        <div v-else-if="planetData?.infoboxes" class="dash-grid">
          <section v-for="(box, i) in planetData.infoboxes" :key="i" class="dash-section" :class="{ 'col-span-full': box.class?.includes('x3') }">
...
async function loadData() {
  loading.value = true
  try {
    const res = await axios.get("/api/planet", { params: route.query })
    planetData.value = res.data.data
...
async function submitPlanetAction(data) {
  loading.value = true
  tooltipModalHtml.value = null // Close modal on action
  try {
    const res = await axios.post("/api/planet", data, { params: route.query })
    planetData.value = res.data.data
    if (res.data.data.messages && res.data.data.messages.length > 0) {
      showToast(res.data.data.messages[0].text, res.data.data.messages[0].type)
    }
    game.fetchDashboard(true) // silent refresh
  } catch(e) {
    console.error(e)
